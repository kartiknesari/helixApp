import { type Plugin, normalizePath, defineConfig } from "vite";
    ...
    return {
        ...
        config(config) {
            ...
            if (!fs.existsSync(better_sqlite3_node)) {
                throw new Error(
                    `${TAG} Can not found "${better_sqlite3_node}".`
                );
            }
            ...
            // Normalize path for .env file (use forward slashes for consistency)
            const BETTER_SQLITE3_BINDING = normalizePath(
                path.relative(resolvedRoot, better_sqlite3_copy)
            );
